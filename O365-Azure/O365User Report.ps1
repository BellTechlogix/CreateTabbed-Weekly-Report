...
$GTILCred = Import-Clixml "C:\scripts\CRD\GTILCred.xml"
#$BTLCred = Import-Clixml "C:\Projects\CRD\BTLCred.xml"

connect-azuread -Credential $GTILCred
...
$mailParams = @{
    SmtpServer                 = $cfg.Settings.EmailSettings.SMTPServer
    From                       = $cfg.Settings.EmailSettings.FromAddress
    To                         = ($cfg.Settings.EmailSettings.ToAddress).split(",")
    Subject                    = "GTIL Monthly - "+$runtime+"-UserList"
    Body                       = "This report contains a monthly automated output of O365/Azure user lists"
    Attachments = $rptFolder+$runtime+'-UserList.csv'
    DeliveryNotificationOption = 'OnFailure', 'OnSuccess'
}

Get-AzureADUser -all $true | select DisplayName,JobTitle,Mail,UserType|export-csv $rptFolder$runtime-UserList.csv -NoTypeInformation
Send-MailMessage @mailParams

Send-MailMessage -SmtpServer 'smtp.belltechlogix.com' -from 'Reports@belltechlogix.com' -to 'kroy@belltechlogix.com' -Subject 'Test'
